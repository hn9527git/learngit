...
void cangku::on_pushButton_clicked()
{
    ...
    case 1:
        ...
        query->prepare("select * from data_cangku where id=:id");
    ...
    case 2:
        name=ui->lineEdit->text().left(2);
        if(name==NULL)
        {
            QMessageBox::critical(this,"错误","请输入要查询的流量计名称","确定");
            break;
        }
        query->prepare("select * from data_cangku where left(name,2)=:name");
...
{
    this->query->prepare("insert into data_cangku (name,number,price) values (:name,:number,:price)");
...
{
    this->query->prepare("select number from data_cangku where name=:name");
    int number=500;
...
//    int pos=re.indexOf("number");
   // query->next();
    ...
    qDebug()<<number;
    query->prepare("update data_cangku set number=:number where name=:name");
...
void cangku::ck_ok_func()
{
    ...
    query->prepare("select number from data_cangku where name=:name");
