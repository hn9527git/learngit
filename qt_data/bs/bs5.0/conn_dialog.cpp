...
//        }
        ...
        //数据库初始化
        ret=true;
        ret=(query->exec("create database if not exists data"));
        ret=(query->exec("use data"));
        ret=(query->exec(" CREATE TABLE if not exists `data_caiji` (\
                           `id` int(11) NOT NULL AUTO_INCREMENT,\
                           `leixing` char(255) DEFAULT NULL,\
                           `shunshi` double(255,1) DEFAULT NULL,\
                           `wendu` double(255,1) DEFAULT NULL,\
                           `yali` double(255,1) DEFAULT NULL,\
                           `create_time` timestamp NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,\
                           PRIMARY KEY (`id`)\
                         ) ENGINE=InnoDB AUTO_INCREMENT=592 DEFAULT CHARSET=utf8;"));
        ret=(query->exec("CREATE TABLE if not exists `data_cangku` (\
        ...
        if(ret!=true)
        {
               QMessageBox::critical(this,"提示","数据库初始化失败，请重试","确定");
                return ;
        }
