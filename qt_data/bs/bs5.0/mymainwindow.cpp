...
void MyMainWindow::on_pushButton_close_clicked()
{
    ...
    if(this->m_dataTimer->isActive())
    {
        this->m_dataTimer->stop();//关闭流量计时关闭采集数据的定时器,不在采集数据
    }
    if(this->ck->serial->isOpen())
    {
...
void MyMainWindow::update_zhen()
{
    ...
    {
        qDebug()<<this->run_low<<"<流量范围>"<<this->run_high;
        qDebug()<<"\xe5\xbd\x93\xe5\x89\x8d\xe6\xb5\x81\xe9\x87\x8f\xef\xbc\x9a"<<this->speed;
        this->status=1;
...
void MyMainWindow::set_range()
{
    ...
    this->run_high=this->set_ptr->get_high();
    //this->sec=this->set_ptr->get_sec();//在别处修改
}
